...
**Step 3 — Start the executor** in a **fresh context** on any *host* — a new chat, a shell loop, a Grok `/goal` session. The host only keeps the node alive across turns; the executor's authority is `executor.md` + the ledger. Hand the host the executor prompt, but keep the ledger and directives as **live files the node re-reads each round** — never fold their content into the host's own goal/prompt text, where it goes stale. A host's progress UI (a goal's done-bar) mirrors the ledger, never replaces it; the ledger wins every conflict. It's self-contained (points at ledger + ops) and runs fine on a **cheap/fast model** — structure, not model, keeps it on-spec.
