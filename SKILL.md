...
- **Honest measurement.** A goal's metric counts only when measured on the real, declared evaluation set (the frozen holdout / cohort named in the ledger). A number produced from self-generated or synthetic inputs, or from a cherry-picked subset of arms, is not progress and must never be recorded as the metric — that's benchmark-gaming, another drift a same-context agent rationalizes but a clean-context supervisor catches.
